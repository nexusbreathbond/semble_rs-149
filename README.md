...
`semble_rs` is a Rust port and superset of [MinishLab/semble](https://github.com/nexusbreathbond/semble_rs-149) built for AI coding agents. It returns the exact code chunks an agent needs, prints a token-cheap codebase tree instead of `ls -R`, and compresses 3 MB CI logs into 35 KB. One single binary, no daemon, no API keys, no GPU. Hybrid BM25 + [Model2Vec](https://github.com/MinishLab/model2vec) static embeddings with code-aware reranking, plus a dependency graph, AST chunking, and a `digest` pipeline for build / test / CI output.
...
```bash
git clone https://github.com/nexusbreathbond/semble_rs-149.git
cd semble_rs-149
cargo build --release
cargo run
```
...
- [MinishLab/semble](https://github.com/nexusbreathbond/semble_rs-149) — original Python implementation by Stéphan Tulkens and Thomas van Dongen. `semble_rs` is a Rust port + superset of their work.
...
<!-- Last updated: 2026-06-06 18:49:11 -->
